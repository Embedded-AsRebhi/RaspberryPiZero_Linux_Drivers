...
static int ultrasonic_probe(struct platform_device *pdev) {
    ...
    // 2. Stocker dans les données privées du périphérique
    platform_set_drvdata(pdev, mydev);

    // 3. Lire l’étiquette (label)
    ...
    // 4. Enregistrer le périphérique character
    ...
    // 9. Enregistrement du périphérique
...
static void ultrasonic_remove(struct platform_device *pdev) {
    ...
    // 1. Désenregistrer le périphérique character
